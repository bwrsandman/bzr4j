...
public abstract class BzrAbstractHandler {
  ...
  private static final DebugLogger LOG = DebugManager.getLogger(BzrAbstractHandler.class);
  ...
  public BzrAbstractHandler addArguments(String... args) {
    return addArguments(Arrays.asList(args));
  }

  public BzrAbstractHandler addArguments(List<String> args) {
    m_args.addAll(args);
    return this;
  }
...
//    logDebug(String.format("(%s) : %s", String.valueOf(workDir), args.toString()));
    LOG.debug(String.format("(%s) : %s", String.valueOf(workDir), args.toString()));
  }
