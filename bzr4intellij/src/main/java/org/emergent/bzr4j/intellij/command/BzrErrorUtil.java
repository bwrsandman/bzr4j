...
final class BzrErrorUtil {
  ...
  static boolean isAbort(ShellCommandResult result) {
    String line = getLastErrorLine(result);
    return StringUtils.isNotBlank(line) && StringUtils.contains(line, "abort:");
  }

  static boolean isAuthorizationRequiredAbort(ShellCommandResult result) {
    String line = getLastErrorLine(result);
  ...
  private static String getLastErrorLine(ShellCommandResult result) {
