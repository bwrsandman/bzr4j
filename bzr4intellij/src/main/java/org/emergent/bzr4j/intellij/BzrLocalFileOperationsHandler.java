...
//        m_vcs = vcs;
    ...
    public File copy( final VirtualFile file, final VirtualFile toDir, final String copyName )
            throws IOException
    {
        sm_logger.info( "copy(" + file + "," + toDir + ");" );
            ...
            throws BazaarException
    {
        ...
        IJUtil.createBzrClient().move( src, dst, options.toArray( new Option[options.size()] ) );
        return true;
    }
    ...
    private void processAddedFiles( Project project )
    {
        ...
        if ( value != VcsShowConfirmationOption.Value.DO_NOTHING_SILENTLY )
        {
            ...
            else
            {
                ...
                {
                    singleFilePrompt = BzrBundle.message( "confirmation.text.add.dir", addedVFiles.get( 0 ).getName() );
                }
                else
                {
                    singleFilePrompt = BzrBundle.message( "confirmation.text.add.file", addedVFiles.get( 0 ).getName() );
                }
            ...
            if ( filesToProcess != null )
            {
                ...
                for ( VirtualFile file : filesToProcess )
                {
                    ...
                    try
                    {
                        IJUtil.createBzrClient().add( new File[]{ioFile}, IAddOptions.NO_RECURSE );
