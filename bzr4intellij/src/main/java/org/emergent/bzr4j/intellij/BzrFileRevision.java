...
public class BzrFileRevision implements VcsFileRevision
{
    ...
    public BzrFileRevision( Project bzr, File file, IBazaarLogMessage message )
    {
        this( BzrVcs.getInstance( bzr ), file, message );
    }
    ...
    public byte[] getContent() throws IOException
    {
        try
        {
            IJUtil.createBzrClient( IJUtil.root( m_file ) );
            InputStream stream = IJUtil.createBzrClient().cat( m_file, m_message.getRevision() );
