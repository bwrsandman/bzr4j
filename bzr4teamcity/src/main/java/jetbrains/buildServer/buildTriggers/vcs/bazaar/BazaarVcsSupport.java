...
public class BazaarVcsSupport extends VcsSupport
  ...
  public String getCurrentVersion(@NotNull VcsRoot root) throws VcsException {
    ...
    try {
      BzrTeamcityHandler handler = new BzrTeamcityHandler(settings,"revno");
      BzrHandlerResult result = handler.exectc();
  ...
  public boolean sourcesUpdatePossibleIfChangesNotFound(@NotNull VcsRoot root) {
    return super.sourcesUpdatePossibleIfChangesNotFound(root);
  }
  ...
  public String testConnection(@NotNull final VcsRoot vcsRoot) throws VcsException {
    ...
    try {
      BzrTeamcityHandler handler = new BzrTeamcityHandler(settings,"info");
...
//    }
  ...
  public List<ModificationData> collectBuildChanges(
    ...
    try {
      ...
      BzrTeamcityHandler handler = new BzrTeamcityHandler(settings, "xmllog", "-r", fromVersion + ".." + currentVersion);
      BzrHandlerResult eres = handler.exectc(true);
      List<ChangeSet> changeSets = TCUtil.parseChangeSets(eres);
  ...
  public LabelingSupport getLabelingSupport() {
    return new LabelingSupport() {
      public String label(@NotNull String label, @NotNull String version, @NotNull VcsRoot root,
        ...
        try {
          Settings settings = createSettings(root);
          (new BzrTeamcityHandler(settings,"tag","-r",version,"--force",label)).exectc(true);
          (new BzrTeamcityHandler(settings,"push","-q",settings.getPushUrl())).exectc(true);
          return label;
        } catch (BzrHandlerException e) {
          throw new VcsException(e);
        }
  ...
  private void buildIncrementalPatch(final Settings settings, @NotNull final ChangeRev fromVer,
      ...
      throws VcsException, IOException, BazaarException, BzrHandlerException {
  ...
  private File doCat(Settings settings, String myRevId, List<String> relPaths)
      throws IOException, BazaarException, BzrHandlerException {
  ...
  private void buildFullPatch(final Settings settings, @NotNull final ChangeRev toVer,
      ...
      throws IOException, VcsException, BazaarException, BzrHandlerException {
    File tempDir = FileUtil.createTempDirectory("bazaar", toVer.getId());
    try {
      final File repRoot = new File(tempDir, "rep");
      BzrTeamcityHandler handler = new BzrTeamcityHandler(settings,"export","-q","--format=dir","-r",toVer.getId(),
...
//    lockWorkDir(workDir);
    try {
      BzrTeamcityHandler handler = null;
      if (settings.hasCopyOfRepository()) {
        handler = new BzrTeamcityHandler(settings, "pull", "-q", "--overwrite", settings.getRepositoryUrl());
      } else {
        handler = new BzrTeamcityHandler(settings, "clone","-q", settings.getRepositoryUrl(), workDir.getAbsolutePath());
      }
      BzrHandlerResult result = handler.exectc();
    } catch (BzrHandlerException e) {
      throw new VcsException(e);
